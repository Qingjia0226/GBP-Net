...
def build_model(cfg, device, rank=None):
    ...
    input = torch.randn(1, 1, 47, 78, 78).to(device)  # FLOPs = 26.65734464G
    ...
    for param in model.conv_in.parameters():
      param.requires_grad = False

    for down_layer in model.down_layers:
    ...
    for down_layer in model.up_layers:
        for param in down_layer.parameters():
            param.requires_grad = False
