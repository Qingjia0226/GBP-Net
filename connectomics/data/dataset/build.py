...
def build_166(train=True):
    ...
    points=[]
    if train:
        filePath = "/home/yanhy/data-zbf/z_train/"
    ...
    for train in pic_list:
        ...
        if '.h5' in train:
            ...
            try:
             ...
             while v.shape[1] < 234:
                segm = np.pad(segm, ((0, 0), (25, 25), (0, 0)), 'constant')
                v = np.pad(v, ((0, 0), (25, 25), (0, 0)), 'constant')

             while v.shape[2] < 234:
                segm = np.pad(segm, ((0, 0), (0, 0), (25, 25)), 'constant')
                v = np.pad(v, ((0, 0), (0, 0), (25, 25)), 'constant')
            except:
                 pass
    ...
    return volume, segmentation,points
...
def get_dataset(cfg,
    ...
    if cfg.DATASET.DO_CHUNK_TITLE == 1:  # build TileDataset
        ...
        dataset = dataset_class(volume=volume, label=label, valid_mask=valid_mask,points=points,
