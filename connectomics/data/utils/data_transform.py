...
def distance_transform(label: np.ndarray,
    ...
    if padding:
        ...
        label = np.pad(label, pad_size, mode='constant', constant_values=1)
    ...
    if not all_bg_sample:
        for idx in indices:
            ...
            #print(np.unique(temp2,return_counts=True))   
            #boundary_edt=np.log2(boundary_edt+1e-2)
            boundary_edt=np.clip(boundary_edt, 0, 6)
            distance=boundary_edt / (boundary_edt.max() + eps) 
