...
def _forward(model, volume,volume_l):
    #output1,output1 = model(volume,volume_l)
    output,output1 = model(volume,volume_l)
...
class TestAugmentor(object):
    ...
    def __call__(self, model, data,volume_l):

        assert len(data.shape) == 5, \
            "The input has a shape of {}, which not a valid 3D " \
            "input in (B, C, Z, Y, X) format.".format(data.shape)
        return self._tta_3d(model, data,volume_l)

    def _tta_3d(self, model, data,volume_l):
        ...
        for xflip, yflip, zflip, transpose in opts:
            ...
            if self.inference_act is not None:
                vout = self.inference_act(
                    _forward(model, volume, volume_l)).detach().cpu()

            else:
                vout = model(_forward(model, volume, volume_l)).detach().cpu()
